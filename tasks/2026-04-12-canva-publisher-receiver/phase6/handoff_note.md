...
1. **`ideation/plans/seeds-index.md`** — Seed 8 항목 + Seed 2(Tier 승계) / Seed 5(P0-② 교사앱 한정) 인접성 파악
2. **`ideation/plans/canva-publisher-receiver-roadmap.md`** — 본 작업의 SSOT. §1 설계 전제(D1~D16) · §2 파일 맵 · §4 CR-1~CR-10 · §5 수용기준 15건
3. **`ideation/plans/tablet-performance-roadmap.md`** §2 성능 예산 — 갤럭시 탭 S6 Lite 교사 UI 터치 타겟·TTI·p95 기준
4. **`ideation/plans/implementation-roadmap.md`** #p0-②-content-publisher-intent 크로스 참조 — Canva 앱(`content-publisher-app/`) SSOT vs 수신측(Seed 8) SSOT 범위 분할
5. **`ideation/tasks/2026-04-12-canva-publisher-receiver/phase3/decisions.md`** — D1~D16 결정 근거 원본
6. **`ideation/tasks/2026-04-12-canva-publisher-receiver/phase4/seed.yaml`** — acceptance_criteria 15건, ontology, evaluation_principles, exit_conditions
7. (선택) `ideation/plans/phase0-requests.md` "Canva Publisher 수신" 섹션 — CR-1~CR-10 작업 카드 블록
...
padlet `feature` 파이프라인이 본 request.json을 phase0 analyst 입력으로 소비한다. CR-1~CR-10 10개 작업 카드는 `ideation/plans/phase0-requests.md`에 별도 등재되어 있으며, 본 핸드오프는 **수용 기준 전건 달성**을 한 트랙으로 묶는 상위 컨텍스트다.
...
- handoff-writer는 padlet 폴더에 쓰기 권한 없음. 본 request.json은 **ideation 내 phase6/** 에만 생성.
