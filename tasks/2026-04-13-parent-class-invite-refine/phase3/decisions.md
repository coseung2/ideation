...
## 6-A. 사용자 사후 확정 (2026-04-13, phase7 dispatch 후 리뷰)

사용자 질문 "승인된 연결은 유지기간이 있는가?" 검토 중 `year_end` revokedReason이 enum에만 정의되고 자동 트리거가 미정의된 구멍 발견. 사용자 확정:

| # | 항목 | 확정값 | 근거 |
|---|---|---|---|
| **D-51** | active 링크 수명 정책 | **학급(Classroom) 존재 기간과 1:1** — 시간 기반 자동 만료 없음 | 교사 운영 모델: 담임은 학급 단위로 관리, 학년 경계보다 학급 실체가 우선 |
| **D-52** | Classroom 삭제 시 cascade | 해당 학급 학생의 모든 `ParentChildLink` 일괄 revoke (status=revoked, revokedReason=`classroom_deleted`) | 단일 트랜잭션 내 처리, 학부모 세션도 즉시 차단 |
| **D-53** | `revokedReason` enum 추가 | **+ `classroom_deleted`** (기존 `year_end`는 유지하되 자동 트리거 없음, 수동 사유로만 존속) | cascade 감사 추적 |
| **D-54** | 학부모 안내 | cascade 시 학부모 이메일 `"[Aura-board] 연결된 학급이 종료되어 액세스가 해제되었습니다"` (교사 이름·사유 비노출) | D-31 거부 이메일 격리 정책 승계 |
| **D-55** | Classroom 삭제 UX | 삭제 확인 모달에 "이 작업은 학부모 N명의 액세스를 해제합니다" 경고 + 확인 문구 입력 | 실수 삭제 방어 |
| **D-56** | year_end Cron 폐지 | **미구현 확정** (기존에도 미구현) — 학년 종료 시 교사는 학급을 Archive/Delete 선택 | D-51 정책의 자연 귀결 |

연쇄 영향:
- Prisma: `ParentChildLink.revokedReason` 값에 `classroom_deleted` 추가
- API: Classroom delete 핸들러에서 트랜잭션으로 cascade (학생 조회 → 해당 학생의 active 링크 전부 revoke + ParentSession 차단)
- 작업 카드: **PV-17 (신규)** Classroom cascade revoke + 삭제 확인 모달 (예상 공수 1.5일)
- roadmap §1.2 Revoke 경로 표에 "Classroom 삭제(cascade)" 행 추가
