...
1. `ideation/plans/seeds-index.md` — Seed 7의 타 Seed 대비 위치와 cross-cutting 의존성 맥락
2. `ideation/plans/parent-viewer-roadmap.md` — §0~§11 본 Seed의 확정 결정·스키마·§5 자녀 범위 매트릭스(SSOT)·PV-1~PV-12 작업 분할
3. `ideation/plans/tablet-performance-roadmap.md` — 스마트폰 PWA/갤럭시 탭 S6 Lite TTI·이미지·썸네일 성능 예산 계승
4. `ideation/plans/drawing-board-library-roadmap.md` — §학부모 열람 범위: StudentAsset studentId ∈ parent.children, presigned 썸네일 규칙
5. `ideation/plans/plant-journal-roadmap.md` — §3.4 학부모 뷰 PJ-8 Seed 7 이관(§5 매트릭스의 PlantObservation 행)
6. `ideation/plans/event-signup-roadmap.md` — §학부모 열람 범위: 학급 이벤트 메타 허용, 자녀 본인 Submission만, 참가자 명단·득점 마스킹
7. `ideation/plans/breakout-room-roadmap.md` — §8 학부모 열람 범위: BreakoutMembership 필터, teacher-pool 제외, 자녀 모둠 한정
8. `ideation/plans/phase0-requests.md` — PV-1~PV-12 padlet 진입 JSON 블록 원본
